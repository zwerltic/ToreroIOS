    ...
    @IBAction func tokenizeCard(sender : AnyObject) {
       ...
       //var tarjeta = Card(last4: "4242424242424242", name: "Julion", cvc: "456", exp_month: "09", exp_year:"2018")
        ...
        conekta.createToken(tarjeta, withSuccess: { (data) -> Void in
            ...
            var messageTok = data["message"] as! String!
            println(token)
            if token != nil {
                ...
                let task = NSURLSession.sharedSession().dataTaskWithRequest(request) {
                    ...
                    if responseString as! String == "Success" {
                        ...
                        var alert = UIAlertController(title:"Pago", message: "La tarjeta no es valida", preferredStyle: UIAlertControllerStyle.Alert)
                        alert.addAction(UIAlertAction(title: "Ok", style: UIAlertActionStyle.Default, handler: {(alert: UIAlertAction!) in
                            self.buttonPagar.setTitle("Pagar", forState: .Normal)
                            self.buttonPagar.enabled = true
                        }))
                ...
                println("Alert is showing")
                var alert = UIAlertController(title:"Hubo un error", message: messageTok , preferredStyle: UIAlertControllerStyle.Alert)
                alert.addAction(UIAlertAction(title: "Ok", style: UIAlertActionStyle.Default, handler: {(alert: UIAlertAction!) in
                    self.buttonPagar.setTitle("Pagar", forState: .Normal)
                    self.buttonPagar.enabled = true
